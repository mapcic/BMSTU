...
\begin{frame}
	...
	\column{0.8\textwidth}
		{\color{blue} Проверка схем:}\\
	\column{0.2\textwidth}
		{\color{blue} Ошибка схем:}\\
	\end{columns}
	\begin{columns}
	\column{0.8\textwidth}
		\includegraphics[width=.99\textwidth,center]{assets/DifFD}\\
	\column{0.01\textwidth}
		\rule[0mm]{0.2ex}{60mm}
	\column{0.2\textwidth}
	\scriptsize
	{\color{red}FTCS}: $O(\Delta x^{2}) + O(\Delta t)$
	\newline\newline
	{\color{red}BTCS}: $O(\Delta x^{2}) + O(\Delta t)$
	\newline\newline
	{\color{red}Crank-Nicolson}: $O(\Delta x^{2}) + O(\Delta t^{2})$
	\end{columns}
	{\color{blue} Вывод:} FTCS -- расходится. BTCS и С-N соответствуют соответствует аналитическому решению. В качестве основной конечно-разностной схемы диффузии принимается BTCS из-за ее простоты и схождением при любых условиях.
\end{frame}
